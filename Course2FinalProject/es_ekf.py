...
var_gnss  = 0.01
var_lidar = 1.00   
# var_lidar: original = 1.00; pt2 = 50.00
...
def measurement_update(sensor_var, p_cov_check, y_k, p_check, v_check, q_check):
    ...
    q_hat = Quaternion(euler=dx_k[6:]).quat_mult_left(q_check, out='np')
...
for k in range(1, imu_f.data.shape[0]):  # start at 1 b/c we have initial prediction from gt
    ...
    acc_prev = C_ns @ imu_f.data[k-1]
    theta_prev = imu_w.data[k-1] * delta_t
    p_check = p_est[k-1] + delta_t * v_est[k-1] + delta_t**2/2 * (acc_prev + g)
    v_check = v_est[k-1] + delta_t * (acc_prev + g)
    q_check = Quaternion(euler=theta_prev).quat_mult_right(q_est[k-1], out='np')
    
    # 1.1 Linearize the motion model and compute Jacobians
    F_prev = np.eye(9)
    F_prev[:3, 3:6] = np.eye(3) * delta_t
    F_prev[3:6, 6:] = -skew_symmetric(acc_prev) * delta_t
    
    # 2. Propagate uncertainty
    q_cov = (delta_t ** 2) * np.eye(6); q_cov[:3, :3] *= var_imu_f; q_cov[3:, 3:] *= var_imu_w;
    p_cov_check = F_prev @ p_cov[k-1] @ F_prev.T + l_jac @ q_cov @ l_jac.T
    
    # 3. Check availability of GNSS and LIDAR measurements
    if gnss_i < len(gnss.t) and gnss.t[gnss_i] <= imu_f.t[k]:
        p_check, v_check, q_check, p_cov_check = measurement_update(var_gnss, p_cov_check, gnss.data[gnss_i], 
                                                                    p_check, v_check, q_check)
        gnss_i += 1
        
    if lidar_i < len(lidar.t) and lidar.t[lidar_i] <= imu_f.t[k]:
        p_check, v_check, q_check, p_cov_check = measurement_update(var_lidar, p_cov_check, lidar.data[lidar_i],
                                                                    p_check, v_check, q_check)
        lidar_i += 1
