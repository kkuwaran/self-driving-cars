    ...
    def collision_check(self, paths, obstacles):
        ...
        for i in range(len(paths)):
            ...
            for j in range(len(path[0])):
                ...
                # --------------------------------------------------------------
                circ_offsets = np.array(self._circle_offsets)
                circle_locations[:, 0] = path[0][j] + circ_offsets * cos(path[-1][j])
                circle_locations[:, 1] = path[1][j] + circ_offsets * sin(path[-1][j])
                # --------------------------------------------------------------
    ...
    def select_best_path_index(self, paths, collision_check_array, goal_state):
        ...
        for i in range(len(paths)):
            ...
            if collision_check_array[i]:
                ...
                # TODO: INSERT YOUR CODE BETWEEN THE DASHED LINES
                # --------------------------------------------------------------
                score = np.linalg.norm([paths[i][0][-1] - goal_state[0], 
                ...
                for j in range(len(paths)):
                    if j == i:
                        ...
                        if not collision_check_array[j]:
                            # TODO: INSERT YOUR CODE BETWEEN THE DASHED LINES
                            # --------------------------------------------------
                            paths_i, paths_j = np.array(paths[i]).T, np.array(paths[j]).T
                            dist_matrix = scipy.spatial.distance.cdist(paths_i, paths_j)
                            score += self._weight * np.mean(dist_matrix)
                            # --------------------------------------------------
